...
class Slab[Content, L <: HList](val content: Content, val annotations: L) {
  ...
  def selectMany[T <: HList](implicit sel: SubSelectMany.Aux[L, T, T]): T = sel(annotations)
  ...
  // questionable.
  def add[A, Tmp <: HList, Result <: HList](newAnnotations: List[A])(implicit adder: Adder.Aux[L, List[A], Result]): Slab[Content, Result] = {
    new Slab(content, adder(annotations, newAnnotations))
  }
  def add[A, Tmp <: HList, Result <: HList](newAnnotation: A)(implicit adder: Adder.Aux[L, List[A], Result]): Slab[Content, Result] = {
    new Slab(content, adder(annotations, List(newAnnotation)))
  }
}
