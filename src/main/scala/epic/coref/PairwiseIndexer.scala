...
object PairwiseIndexer {
  def apply(feat: PairwiseFeaturizer, instances: IndexedSeq[CorefInstance]):PairwiseIndexer = {
    ...
    val indexed = for(inst <- instances) yield {
      ...
      val array = new TriangularArray[SparseVector[Double]](length+1, {(a:Int, b:Int) =>
        if (a == 0 && b == 0) null
        else {
          val ctr = if(a == 0) {
    ...
    val fixed  = indexed.map(inst => inst.copy(features = new TriangularArray[SparseVector[Double]](inst.numMentions+1, {(a,b) =>
      if(a == 0 && b == 0) null
      else {
