...
class WordShapeFeaturizer(wordCounts: Counter[String, Double],
  ...
  private val shapes =  Encoder.fromIndex(wordIndex).tabulateArray(w => if(wordCounts(w) > functionWordThreshold) interner(IndicatorFeature(wordIndex(w))) else interner(WordFeature(WordShapeGenerator(w), 'Shape)))
