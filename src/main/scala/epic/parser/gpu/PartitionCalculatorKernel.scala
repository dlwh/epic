 ...
 * @author dlwh
 */
class PartitionCalculatorKernel[L](rules: RuleStructure[L])(implicit context: CLContext) {
  ...
  def partitions(insideTop: CLBuffer[JFloat], offsets: CLBuffer[JInt], lengths: CLBuffer[JInt], numSentences: Int, events: CLEvent*)(implicit queue: CLQueue) = synchronized {
    val partitions = context.createFloatBuffer(CLMem.Usage.Output, numSentences)
    kernel.setArgs(partitions, insideTop, offsets, lengths)
    val toWait = kernel.enqueueNDRange(queue, Array(numSentences), events:_*)
  ...
  __kernel void compute_partitions(
    ...
    partitions[sentence] = log(root_score) - SCALE_FACTOR * LOG_2 * (length-1)
