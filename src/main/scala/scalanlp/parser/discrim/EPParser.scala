  ...
  def buildAllCharts(words: Seq[W], initScorer: SpanScorer=SpanScorer.identity, tree: BinarizedTree[L]=null):Array[ParsedSentenceData] = {
    ...
    for(i <- 0 until maxEPIterations if changed) {
      ...
      for(m <- 0 until parsers.length) {
        ...
        currentF0 = approximators(m).project(insideCharts(m),outsideCharts(m), newPartition, tree); // TODO need projected scorer
        val newCorrection = approximators(m).updateCorrection(corrections(m),currentF0,rescaledScorer,words.length,damping);
        val oldCorrection = corrections(m)
        val maxChange = computeMaxChange(newCorrection,oldCorrection,words.length);
        println("rawr? " + i + " " +  maxChange);
        corrections(m) = newCorrection
