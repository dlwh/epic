...
class ProductParserTest extends FunSuite with ParserTestHarness {
  ...
  test("two parsers test") {
    ...
    val product = Parser(grammar.topology, grammar.lexicon, ParserTestHarness.simpleParser.constraintsFactory, new ProductChartFactory(IndexedSeq(grammar, grammar)))
